...
####################################################################################################################################################################
		myplot <- ggplot(data, aes(V3, V4, colour = V5_new)) + geom_bar(stat = "identity") + facet_wrap(~V5_new, ncol = 1, labeller = labeller(V5_new = labels))
				 
				 #Separate strands and specify y-axis limit		 
				 xlab("Chromosome Coordinate") + ylab("rNMP Coverage") + scale_x_continuous(expand = c(0.015, 0)) + scale_colour_manual(values = c("blue", "green3")) +

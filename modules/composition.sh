...
###################################################################################################################################################################
	...
	#Calculate counts of each nucleotide
	A_Ribo=$(awk '$1 == "A" || $1 == "a"' $output/${sample}.nucs.tab | wc -l)
	C_Ribo=$(awk '$1 == "C" || $1 == "c"' $output/${sample}.nucs.tab | wc -l)
	G_Ribo=$(awk '$1 == "G" || $1 == "g"' $output/${sample}.nucs.tab | wc -l)
	U_Ribo=$(awk '$1 == "T" || $1 == "t"' $output/${sample}.nucs.tab | wc -l)
	...
	#Save nucleotide counts to .txt file
	paste <(echo -e "rA") <(echo "$A_Ribo") > $output/${sample}.counts.txt
	paste <(echo -e "rC") <(echo "$C_Ribo") >> $output/${sample}.counts.txt
	paste <(echo -e "rG") <(echo "$G_Ribo") >> $output/${sample}.counts.txt
	paste <(echo -e "rU") <(echo "$U_Ribo") >> $output/${sample}.counts.txt
		
	#Save nucleotide frequencies to .txt file
	paste <(echo -e "rA") <(echo "$A_RiboFreq2" | xargs printf "%.*f\n" 5) > $output/${sample}.frequencies.txt
	paste <(echo -e "rC") <(echo "$C_RiboFreq2" | xargs printf "%.*f\n" 5) >> $output/${sample}.frequencies.txt
	paste <(echo -e "rG") <(echo "$G_RiboFreq2" | xargs printf "%.*f\n" 5) >> $output/${sample}.frequencies.txt
	paste <(echo -e "rU") <(echo "$U_RiboFreq2" | xargs printf "%.*f\n" 5) >> $output/${sample}.frequencies.txt

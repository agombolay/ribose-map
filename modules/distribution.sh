...
awk -v "OFS=\t" '$5 == "+" {print $1,$2,$3,$4}' $repository/results/$sample/coordinates/$sample.counts.bed >> $output/$sample-Forward.bg

#Rearrange reverse strand file so format is the same as bedgraph format
awk -v "OFS=\t" '$5 == "-" {print $1,$2,$3,$4}' $repository/results/$sample/coordinates/$sample.counts.bed >> $output/$sample-Reverse.bg

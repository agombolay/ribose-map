...
####################################################################################################################################################################
	...
	for nuc in "A" "C" "G" "U" "Combined"; do
		...
		#Calculate index
		index=$(echo "$(wc -l < $repository/results/$sample/coordinate$quality/${sample}.$nuc.tab)*$percentile" | bc)
		...
		if [[ $integer ]]; then
			...
			#Get counts of rNMPs for lines one and two
			value1=$(head -${line1} $repository/results/$sample/coordinate$quality/${sample}.$nuc.tab | tail -1 | awk '{ print $7 }')
			value2=$(head -${line2} $repository/results/$sample/coordinate$quality/${sample}.$nuc.tab | tail -1 | awk '{ print $7 }')
			...
			#Calculate Xth percentile (count at the line)
			threshold=$(head -${line} $repository/results/$sample/coordinate$quality/${sample}.$nuc.tab | tail -1 | awk '{ print $7 }')

...
######################################################################################################################################################################################################
	...
	if [[ $integer ]]; then
		...
		value1=$(head -${line1} $repository/results/$sample/coordinate$quality/${sample}-$region.counts.tab | tail -1 | awk '{ print $5 }')
		value2=$(head -${line2} $repository/results/$sample/coordinate$quality/${sample}-$region.counts.tab | tail -1 | awk '{ print $5 }')
		...
		percentile=$(head -${line} $repository/results/$sample/coordinate$quality/${sample}-$region.counts.tab | tail -1 | awk '{ print $5 }')
...
######################################################################################################################################################################################################
	...
	awk -v "OFS=\t" -v "x=$percentile" '{if ($5 >= x) print $0}' $repository/results/$sample/coordinate$quality/${sample}-$region.counts.tab | awk -v "OFS=\t" '{ print $1, $2, $3, ".", ".", $4 }' - > $output/${sample}-$region.top.bed

...
if [[ ! $read2 ]]; then
	...
	time samtools view -b $quality -@ $threads $repository/results/$sample/alignment/$sample.bam > $output/temp.bam
...
time bedtools bamtobed -i $output/temp.bam > $output/temp1.bed

#Create FASTA index file and BED file for reference
time samtools faidx $fasta && cut -f 1,2 $fasta.fai > $output/reference.bed
...
if [[ $technique == "ribose-seq" ]]; then
	...
	time join -t $'\t' $output/reference.bed <(sort -k1,1 -k2,2n -k 6 $output/temp2.bed) | awk -v "OFS=\t" '$2 >= $4 { print $1, $3, $4, $5, $6, $7 }' > $output/$sample.bed
...
time total=$(wc -l < $output/$sample.bed)
time cut -f1,2,3,6 $output/$sample.bed | uniq -c - | awk -v "OFS=\t" '{print $2, $3, $4, $5, $1}' > $output/$sample.counts.tab
time awk -v "OFS=\t" -v total="$total" '{print $1, $2, $3, $4, $5/total*100}' $output/$sample.counts.tab > $output/$sample.normalized.tab

#############################################################################################################################
#Remove temporary files
time rm -f $output/temp{1..2}.bed $output/temp.bam

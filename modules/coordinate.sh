...
if [[ ! $read2 ]]; then
	...
	samtools view -b -F2308 $quality -@ 2 $repository/results/$sample/alignment/$sample.bam | bedtools bamtobed -i stdin > $output/temp1.bed

elif [[ $read2 ]]; then
	#Keep first read in pair
	samtools view -b -f67 -F2308 $quality -@ 2 $repository/results/$sample/alignment/$sample.bam | bedtools bamtobed -i stdin > $output/temp1.bed
fi

...
if [[ ! $read2 ]]; then
	...
	if [[ ! $pattern ]]; then
		...
		time bowtie2 --threads $threads -x $basename -U $read1 -S $output/aligned.sam 2> $output/alignment.log
		time samtools view -b -S -@ $threads $output/aligned.sam | samtools sort - -@ $threads -o $output/$sample.bam
		time samtools index $output/$sample.bam	

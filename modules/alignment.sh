...
if [[ ! $read2 ]]; then
	...
	if [[ ! $pattern ]]; then
		...
		bowtie2 -N $mismatches -x $basename -U $read1 -S $output/aligned.sam 2> $output/alignment.log
		...
		if [[ ! $barcode ]]; then
			...
			bowtie2 -N $mismatches -x $basename -U $output/extracted1.fq -S $output/aligned.sam 2> $output/alignment.log
			...
			bowtie2 -N $mismatches -x $basename -U $output/demultiplexed1.fq -S $output/aligned.sam 2> $output/alignment.log
			samtools view -bS $output/aligned.sam | samtools sort - -o $output/sorted.bam
			samtools index $output/sorted.bam
	...
	if [[ ! $pattern ]]; then
		...
		bowtie2 -N $mismatches -x $basename -1 $read1 -2 $read2 -S $output/aligned.sam 2> $output/alignment.log
		samtools view -bS $output/aligned.sam | samtools sort - -o $output/$sample.bam
		samtools index $output/$sample.bam
		...
		if [[ ! $barcode ]]; then
			...
			bowtie2 -N $mismatches -x $basename -1 $output/extracted1.fq -2 $output/extracted2.fq -S $output/aligned.sam 2> $output/alignment.log
			samtools view -bS $output/aligned.sam | samtools sort - -o $output/sorted.bam
			samtools index $output/sorted.bam
			...
			bowtie2 -N $mismatches -x $basename -1 $output/demultiplexed1.fq -2 $output/extracted2.fq -S $output/aligned.sam 2> $output/alignment.log
			samtools view -bS $output/aligned.sam | samtools sort - -o $output/sorted.bam
			samtools index $output/sorted.bam

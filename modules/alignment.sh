...
if [[ ! $read2 ]]; then
	...
	if [[ ! $pattern ]]; then
		...
		if [[ ! $barcode ]]; then
			...
			bowtie2 -N $mismatches --seed 123456789 --threads $threads -x $basename -U $output/extracted1.fq -S $output/aligned.sam 2> $output/alignment.log
			...
			bowtie2 -N $mismatches --seed 123456789 --threads $threads -x $basename -U $output/demultiplexed1.fq -S $output/aligned.sam 2> $output/alignment.log
			samtools view -bS -@ $threads $output/aligned.sam | samtools sort - -@ $threads -o $output/sorted.bam
			samtools index $output/sorted.bam
	...
	if [[ ! $pattern ]]; then
		...
		if [[ ! $barcode ]]; then
			...
			bowtie2 -N $mismatches --seed 123456789 --threads $threads -x $basename -1 $output/extracted1.fq -2 $output/extracted2.fq -S $output/aligned.sam 2> $output/alignment.log
			samtools view -bS -@ $threads $output/aligned.sam | samtools sort - -@ $threads -o $output/sorted.bam
			samtools index $output/sorted.bam
			...
			bowtie2 -N $mismatches --seed 123456789 --threads $threads -x $basename -1 $output/demultiplexed1.fq -2 $output/extracted2.fq -S $output/aligned.sam 2> $output/alignment.log
			samtools view -bS -@ $threads $output/aligned.sam | samtools sort - -@ $threads -o $output/sorted.bam
			samtools index $output/sorted.bam

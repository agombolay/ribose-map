	...
	for region in $units "chromosomes"; do
		...
		for nuc in "A" "C" "G" "U" "Combined"; do
			...
			if [[ $nuc == "A" ]]; then
				...
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $repository/results/$sample/coordinate$quality/${sample}-$region.Combined.tab | awk '$2 == "A" || $2 == "a"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$region.$nuc.bed
				...
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $repository/results/$sample/coordinate$quality/${sample}-$region.Combined.tab | awk '$2 == "C" || $2 == "c"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$region.$nuc.bed
				...
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $repository/results/$sample/coordinate$quality/${sample}-$region.Combined.tab | awk '$2 == "G" || $2 == "g"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$region.$nuc.bed
				...
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $repository/results/$sample/coordinate$quality/${sample}-$region.Combined.tab | awk '$2 == "T" || $2 == "t"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$region.$nuc.bed
...
#############################################################################################################################
	...
	#Extract rNMP nucleotides from FASTA
	bedtools getfasta -s -fi $fasta -bed $repository/results/$sample/coordinate$quality/${sample}.Combined.tab | grep -v '>' > $output/${sample}.ribos.txt
			
	#Calculate counts of rNMPs
	A_Ribo=$(awk '$1 == "A" || $1 == "a"' $output/${sample}.ribos.txt | wc -l)
	C_Ribo=$(awk '$1 == "C" || $1 == "c"' $output/${sample}.ribos.txt | wc -l)
	G_Ribo=$(awk '$1 == "G" || $1 == "g"' $output/${sample}.ribos.txt | wc -l)
	U_Ribo=$(awk '$1 == "T" || $1 == "t"' $output/${sample}.ribos.txt | wc -l)
	...
	for nuc in "A" "C" "G" "U" "Combined"; do
		...
		#Save frequencies of rNMPs to TXT files
		if [[ $nuc == "A" ]]; then
			echo $A_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-Ribo$nuc.$nuc.txt
			echo 'NA' > $output/${sample}-Ribo$nuc.C.txt
			echo 'NA' > $output/${sample}-Ribo$nuc.G.txt
			echo 'NA' > $output/${sample}-Ribo$nuc.U.txt
				
			#Create BED file for only A ribonucleotide
			bedtools getfasta -s -fi $fasta -tab -bed $repository/results/$sample/coordinate$quality/${sample}.Combined.tab | awk '$2 == "A" || $2 == "a"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$nuc.bed

		elif [[ $nuc == "C" ]]; then
			echo $C_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-Ribo$nuc.$nuc.txt
			echo 'NA' > $output/${sample}-Ribo$nuc.A.txt
			echo 'NA' > $output/${sample}-Ribo$nuc.G.txt
			echo 'NA' > $output/${sample}-Ribo$nuc.U.txt
				
			#Create BED file for only C ribonucleotide
			bedtools getfasta -s -fi $fasta -tab -bed $repository/results/$sample/coordinate$quality/${sample}.Combined.tab | awk '$2 == "C" || $2 == "c"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$nuc.bed
			...
			#Create BED file for only G ribonucleotide
			bedtools getfasta -s -fi $fasta -tab -bed $repository/results/$sample/coordinate$quality/${sample}.Combined.tab | awk '$2 == "G" || $2 == "g"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$nuc.bed
			...
			#Create BED file for only U ribonucleotide
			bedtools getfasta -s -fi $fasta -tab -bed $repository/results/$sample/coordinate$quality/${sample}.Combined.tab | awk '$2 == "T" || $2 == "t"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-Ribo.$nuc.bed

		elif [[ $nuc == "Combined" ]]; then
			echo $A_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-RiboCombined.A.txt
			echo $C_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-RiboCombined.C.txt
			echo $G_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-RiboCombined.G.txt
			echo $U_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-RiboCombined.U.txt
...
#############################################################################################################################
		...
		#Continue only for BED files > 0
		if [[ -s $output/${sample}-Ribo.$nuc.bed ]]; then
			
			#Obtain coordinates of flanking sequences and remove coordinates where start = end
			bedtools flank -i $output/${sample}-Ribo.$nuc.bed -s -g $(dirname $fasta)/$(basename $fasta .fa).chrom.sizes -l 100 -r 0 | awk '$2 != $3' > $output/${sample}-Upstream.$nuc.bed
			bedtools flank -i $output/${sample}-Ribo.$nuc.bed -s -g $(dirname $fasta)/$(basename $fasta .fa).chrom.sizes -l 0 -r 100 | awk '$2 != $3' > $output/${sample}-Downstream.$nuc.bed
	
			#Obtain nucleotides flanking rNMPs (reverse order of upstream) and insert tabs bases for easier parsing
			bedtools getfasta -s -fi $fasta -bed $output/${sample}-Upstream.$nuc.bed | grep -v '>' | rev | sed 's/.../& /2g;s/./& /g' > $output/${sample}-Upstream.$nuc.tab
			bedtools getfasta -s -fi $fasta -bed $output/${sample}-Downstream.$nuc.bed | grep -v '>' | sed 's/.../& /2g;s/./& /g' > $output/${sample}-Downstream.$nuc.tab
			...
			for direction in "Upstream" "Downstream"; do
				...
				for i in {1..100}; do
					...
					#Save dNMPs frequencies to .txt files
					if [[ $A_FlankFreq == 'NA' ]]; then
						echo $A_FlankFreq >> $output/${sample}-$direction.$nuc.A.txt
					else
						echo $A_FlankFreq | xargs printf "%.*f\n" 5 >> $output/${sample}-$direction.$nuc.A.txt
					fi
					...
					if [[ $T_FlankFreq != 'NA' ]]; then
						echo $T_FlankFreq >> $output/${sample}-$direction.$nuc.T.txt
					else
						echo $T_FlankFreq | xargs printf "%.*f\n" 5 >> $output/${sample}-$direction.$nuc.T.txt
					fi

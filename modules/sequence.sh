...
for file in $output/${sample}-*.bed; do
	...
	if [ -s $file ]; then
		...
		for nuc in "A" "C" "G" "U" "Combined"; do
			...
			if [[ $nuc == "A" ]]; then
				echo $A_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-$region.$nuc.A_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.C_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.G_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.U_Ribo.txt
				
				#Create BED file for only A ribonucleotide
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $file | awk '$2 == "A" || $2 == "a"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-$region.$nuc.A.bed

			elif [[ $nuc == "C" ]]; then
				echo $C_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-$region.$nuc.C_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.A_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.G_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.U_Ribo.txt
				
				#Create BED file for only C ribonucleotide
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $file | awk '$2 == "C" || $2 == "c"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-$region.$nuc.C.bed
				
			elif [[ $nuc == "G" ]]; then
				echo $G_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-$region.$nuc.G_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.A_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.C_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.U_Ribo.txt
				
				#Create BED file for only G ribonucleotide
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $file | awk '$2 == "G" || $2 == "g"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-$region.$nuc.G.bed

			elif [[ $nuc == "U" ]]; then
				echo $U_RiboFreq | xargs printf "%.*f\n" 5 > $output/${sample}-$region.$nuc.U_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.A_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.C_Ribo.txt
				echo 'NA' > $output/${sample}-$region.$nuc.G_Ribo.txt
				
				#Create BED file for only U ribonucleotide
				bedtools getfasta -s -fi $(dirname $fasta)/$(basename $fasta .fa)-$region.fa -tab -bed $file | awk '$2 == "T" || $2 == "t"' | cut -f1 | sed 's/\:/\t/' | sed 's/\-/\t/' | sed 's/(/\t.\t.\t/;s/)//' > $output/${sample}-$region.$nuc.U.bed

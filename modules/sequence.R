...
####################################################################################################################################################################
			combined <- ggplot(data, aes(x = position)) + xlab("Chromosome Position") + ylab("Nucleotide Frequency") +
					   ...
					   theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "black"), axis.line=element_line(colour = "black")) +				   #Replace default background with black lines for the axes

...
if [[ $other ]]; then
	...
	for nuc in "A" "C" "G" "U" "Combined"; do
		...
		if [[ $nuc == "A" ]]; then
			...
			cut -f1,2,3,6 $repository/results/$sample/coordinate$quality/${sample}-Ribo.chromosomes.$nuc.bed | uniq -c - | awk -v "OFS=\t" '{print $2, $3, $4, ".", ".", $5, $1}' - | sort -k7,7n - > $repository/results/$sample/coordinate$quality/${sample}-chromosomes.counts.tab
	...
	for region in $other; do
		...
		for nuc in "A" "C" "G" "U" "Combined"; do
			...
			if [[ $nuc == "A" ]]; then
				...
				cut -f1,2,3,6 $repository/results/$sample/coordinate$quality/${sample}-Ribo.$region.$nuc.bed | uniq -c - | awk -v "OFS=\t" '{print $2, $3, $4, ".", ".", $5, $1}' - | sort -k7,7n - > $repository/results/$sample/coordinate$quality/${sample}-$region.counts.tab

...
for sample in ${sample[@]}; do
	...
	if [[ $type == "SE" ]]; then
		...
		bowtie2 -x $directory/Indices/$index -U $output/Read1.fq 2> $output/Bowtie2.log -S $output/mapped.sam
	...
	if [[ $type == "PE" ]]; then
		...
		#Align reads to reference genome and save Bowtie2 statistics log file
		bowtie2 -x $directory/Indices/$index --no-mixed --no-discordant -1 $output/Paired1.fq \
		-2 $output/Paired2.fq 2> $output/Bowtie2.log -S $output/mapped.sam

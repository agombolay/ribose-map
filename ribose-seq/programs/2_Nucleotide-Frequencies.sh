...
awk '$2 == "+" {print substr($0,length($0)-2)}' temporary.txt > $output1/$sample.Ribonucleotide_List.$subset.txt

#Print ribonucleotides for negative strands (located at beginning of sequence)
awk -v "OFS=\t" '$2 == "-" {print substr($0,0,1), $2}' temporary.txt >> $output1/$sample.Ribonucleotide_List.$subset.txt
